...
function MessageContent({ content, isUser }: MessageContentProps) {
  const parseContent = (text: string): React.ReactNode[] => {
    ...
    codeBlocks.forEach((block, blockIndex) => {
      ...
      parts.push(
        <div
          key={`code-${blockIndex}`}
          className="my-4 rounded-lg bg-muted/50 border border-border/50 overflow-hidden"
        >
          {block.language && (
  ...
  const renderTable = (
    ...
    const hasHeader =
      tableData.length > 1 &&
      tableData[0].every(
        cell => !cell.match(/^\d{4}-\d{2}-\d{2}/) && !cell.match(/^\d+\.?\d*$/)
      )
    return (
        ...
        <table className="w-full text-xs border border-border/50 rounded-lg overflow-hidden">
          ...
          <tbody>
            {(hasHeader ? tableData.slice(1) : tableData).map((row, i) => (
              <tr
                key={i}
                className={`${i % 2 === 0 ? 'bg-background/50' : 'bg-muted/20'} hover:bg-accent/10 transition-colors`}
              >
                {row.map((cell, j) => (
                  <td
                    key={j}
          ...
          .map((linkPart, linkIndex) => {
            if (linkIndex % 4 === 0) {
              ...
              if (urlMatch && urlMatch[Math.floor(linkIndex / 4)]) {
                const url =
                  urlMatch[Math.floor(linkIndex / 4)].match(/\(([^)]+)\)/)?.[1]
                return (
                  <a
                    key={`${index}-${linkPart}`}
                    href={url}
...
function App() {
  ...
  const _isAnyTyping = chatMessages.some(m => m.isTyping)
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Silently ignore errors when saving conversations
    }
  ...
  const loadConversation = async (conversation: ConversationEntry) => {
    try {
      ...
      const response = await fetch(
        `/api/thread/${conversation.thread_id}/messages`
      )
      if (response.ok) {
        const result = await response.json()
        if (result.success && result.messages) {
          const loadedMessages: ChatMessage[] = result.messages.map(
            (msg: any, index: number) => ({
              ...
              agent:
                msg.role === 'assistant'
                  ? conversation.agent_type === 'genie'
                    ? 'genie'
                    : conversation.agent_type === 'fabric'
                      ? 'fabric'
                      : conversation.agent_type === 'rag'
                        ? 'rag'
                        : conversation.agent_type === 'web'
                          ? 'web'
                          : 'purview'
                  : undefined,
  ...
  const clearAllConversations = () => {
    try {
      ...
      // Silently ignore errors when clearing conversations
    }
  }

  const extractAgentFromAnalysis = (
    purviewMessage: string
  ): 'fabric' | 'rag' | 'web' | 'genie' | null => {
    if (!purviewMessage) return null
    const lowerMessage = purviewMessage.toLowerCase()
    if (
      lowerMessage.includes('fabric') ||
      lowerMessage.includes('fabric_agent')
    ) {
      return 'fabric'
    } else if (
      lowerMessage.includes('rag') ||
      lowerMessage.includes('rag_agent') ||
      ...
      lowerMessage.includes('bing')
    ) {
      return 'web'
    } else if (
      lowerMessage.includes('genie') ||
      lowerMessage.includes('databricks')
    ) {
      return 'genie'
    }
    return null
  ...
  const extractActualAgentFromResponse = (
    processingResult: any
  ): 'fabric' | 'rag' | 'web' | 'genie' | null => {
    const toolsCalled = processingResult?.metadata?.tools_called || []
    if (
      toolsCalled.some((tool: string) => tool.includes('handoff_genie_agent'))
    ) {
      return 'genie'
    }
    const connectedAgentsCalled =
      processingResult?.metadata?.connected_agents_called || []
    if (connectedAgentsCalled.length > 0) {
      ...
      if (actualAgent === 'web-agent' || actualAgent === 'web_agent') {
        ...
        actualAgent === 'fabric-agent' ||
        actualAgent === 'fabric_agent'
      ) {
        return 'fabric'
      }
    }
    ...
    try {
      ...
      if (selectedMode !== 'auto') {
        ...
        switch (selectedMode) {
          case 'fabric':
            routingExplanation =
              'User selected Fabric Data Agent. Routing directly to structured data analysis via NL2SQL.'
            break
          case 'rag':
            routingExplanation =
              'User selected RAG Agent. Routing to document processing and semantic search.'
            break
          case 'web':
            routingExplanation =
              'User selected Web Search. Routing to Bing search for real-time information.'
            break
          case 'genie':
            routingExplanation =
              'User selected Databricks Genie Agent. Routing to natural language data analysis.'
            break
        }
      ...
      await typeMessage(
        purviewMessageId,
        `Analyzing query to determine optimal data source and processing method...`,
        15
      )
      updateMessage(purviewMessageId, { isTyping: true })
      ...
      if (analysisResult.catalog_results?.assets_found > 0) {
        ...
        const agentTypes = assets
          .map((asset: any) => asset.connected_agent)
          .filter(Boolean)
      ...
      await processWithConnectedAgents(
        userQuery,
        analysisResult,
        purviewMessageId
      )
  ...
  const processDirectly = async (
    ...
    try {
      ...
      if (!directResponse.ok) {
        throw new Error(
          `Direct processing failed: ${directResponse.statusText}`
        )
      }
      ...
      const threadId =
        directResult.metadata?.thread_id || currentThreadId || 'unknown'
  ...
  const processWithConnectedAgents = async (
    ...
    try {
      ...
      if (
        processingResult.annotations &&
        processingResult.annotations.length > 0
      ) {
        processingResult.annotations.forEach(
      ...
      const threadId =
        processingResult.metadata?.thread_id || currentThreadId || 'unknown'
      saveConversation(
        userQuery,
          ...
          <div className="flex items-center gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gradient">
                Agent Router Demo App
              </h1>
              <div className="flex items-center gap-2">
                <p className="text-sm text-muted-foreground">
                  Purview-powered intent routing
                </p>
                {currentConversationId && (
                  <Badge
                    variant="outline"
                    className="text-xs px-2 py-1 bg-accent/10 text-accent border-accent/30"
                  >
                    Active conversation
                  </Badge>
                )}
                                ...
                                >
                                  {conv.agent_type === 'fabric'
                                    ...
                                    : conv.agent_type === 'rag'
                                      ...
                                      : conv.agent_type === 'web'
                                        ? 'Web Agent'
                                        : conv.agent_type === 'genie'
                                          ? 'Genie Agent'
                                          : conv.agent_type === 'auto'
                                            ? 'Auto Route'
                                            : 'Auto Route'}
                                </Badge>
                ...
                >
                  <div className="text-center max-w-2xl">
                    ...
                    <p className="text-lg text-muted-foreground leading-relaxed mb-8">
                      Route your queries through Microsoft Purview to find the
                      right data source and specialist agent. Whether you need
                      structured analytics, document processing, or real-time
                      information, we'll connect you to the optimal solution.
                    </p>
                    <div className="flex flex-wrap justify-center gap-3">
                      <Badge
                        variant="outline"
                        className="px-4 py-2 bg-red-500/15 text-red-400 border-red-400/30"
                      >
                        Structured Data • Databricks
                      </Badge>
                      <Badge
                        variant="outline"
                        className="px-4 py-2 bg-white/10 text-white border-white/30"
                      >
                        Unstructured Docs • RAG
                      </Badge>
                      <Badge
                        variant="outline"
                        className="px-4 py-2 bg-orange-500/15 text-orange-400 border-orange-400/30"
                      >
                        Real-Time Info • Web
                      </Badge>
                    </div>
                            ...
                            {message.isTyping && (
                              <div
                                className={`loading-border ${
                                  getAgentColor(message) === 'user'
                                    ? 'loading-border-user'
                                    : getAgentColor(message) === 'purview'
                                      ? 'loading-border-purview'
                                      : getAgentColor(message) === 'fabric'
                                        ? 'loading-border-fabric'
                                        : getAgentColor(message) === 'rag'
                                          ? 'loading-border-rag'
                                          : getAgentColor(message) === 'web'
                                            ? 'loading-border-web'
                                            : getAgentColor(message) === 'genie'
                                              ? 'loading-border-genie'
                                              : ''
                                }`}
                              />
                            )}
              ...
              <form onSubmit={handleSubmit} className="flex gap-3 items-end">
                <div className="flex-1 space-y-2">
                    ...
                    <DropdownMenu>
                      ...
                      >
                        ...
                        <DropdownMenuItem
                          ...
                          className={
                            selectedMode === 'auto' ? 'bg-accent/20' : ''
                          }
                        >
                        ...
                        {fabricAgentEnabled && (
                          <DropdownMenuItem
                            ...
                            className={
                              selectedMode === 'fabric' ? 'bg-primary/20' : ''
                            }
                          >
                        ...
                        <DropdownMenuItem
                          ...
                          className={
                            selectedMode === 'genie' ? 'bg-accent/20' : ''
                          }
                        >
                        ...
                        <DropdownMenuItem
                          ...
                          className={
                            selectedMode === 'rag' ? 'bg-secondary/20' : ''
                          }
                        >
                          <FileText
                            size={16}
                  ...
                  <Input
                    ...
                    placeholder={
                      selectedMode === 'fabric'
                        ? 'Ask about customer data, sales analytics, reports...'
                        : selectedMode === 'rag'
                          ? 'Ask about invoices, documents, contracts...'
                          : selectedMode === 'web'
                            ? 'Ask about current events, general knowledge...'
                            : selectedMode === 'genie'
                              ? 'Ask about data analysis, SQL queries, insights...'
                              : fabricAgentEnabled
                                ? 'Ask about customer data, invoice analysis, reports...'
                                : 'Ask about data analysis, documents, web search...'
                    }
