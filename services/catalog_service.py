...
class CatalogService:
    ...
    def parse_agent_from_description(self, description: str) -> Optional[str]:
        match = (
            re.search(r"agent:\s*(\w+)", description, re.IGNORECASE)
            if description
            else None
        )
    ...
    def search_catalog(self, query: str) -> Dict[str, Any]:
        ...
        for asset in search_response.json().get("value", []):
            ...
            contact_id = (
                asset.get("contact", [{}])[0].get("id")
                if asset.get("contact")
                else None
            )
        ...
        return {"status": "success", "assets_found": len(results), "results": results}
