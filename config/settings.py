...
class Settings:
    def __init__(self):
        ...
        self.ENABLE_FABRIC_AGENT = (
            os.getenv("ENABLE_FABRIC_AGENT", "false").lower() == "true"
        )
    ...
    def validate(self) -> Dict[str, Any]:
        missing_vars = [
            var for var in self.get_required_vars() if not getattr(self, var)
        ]
        return {
            "valid": len(missing_vars) == 0,
            "missing_variables": missing_vars,
            "fabric_enabled": self.ENABLE_FABRIC_AGENT,
            "genie_configured": all(
                [
                    self.DATABRICKS_INSTANCE,
                    self.GENIE_SPACE_ID,
                    self.DATABRICKS_AUTH_TOKEN,
                ]
            ),
        }
